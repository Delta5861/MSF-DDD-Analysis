...
parse_pharmacy_data <- function(target_dir) {
  ...
  for (sheet in all_sheets) {
    ...
    # Remove drugs based on the extended description
    df_ipd <- df_ipd  %>% filter(!grepl("disp|susp|eye", Description))
...
# Next, merge together the Amoxicillin entries (e.g. 0.25g, 0.5g, 1g)
...
process_pharmacy <- function(location) {
  ...
  # 
  merge_amox <- pharmacy_data %>% filter(grepl("AMOXICILLIN[E]* [0-9]", Description))
  if (nrow(merge_amox) != 0) {
    merge_amox[1,-1] <- colSums(merge_amox[,-1], na.rm = T)
    merge_amox[1,1] <- "AMOXICILLIN"
  
    pharmacy_data <- bind_rows(merge_amox[1,], pharmacy_data) %>% filter(!grepl("AMOXICILLIN[E]* [0-9]", Description))
  }
...
quarterly_AWARE_plots <- function(pharmacy_data, monthly_data, since = NA, location, category="ALL") {
  ...
  if (category == "ALL") {
    plt <- ggplot(combined, aes(x=quarters, y=DDD_1k)) +
...
broadcast_departments <- TRUE       # Whether to print all available departments per consumption tool
...
clox_He  <- plot_by_drug(pharmacy_Helmand, monthly_Helmand, "CLOXACILLIN 0.25g", since=30, "Helmand")
quart_He <- quarterly_AWARE_plots(pharmacy_Helmand, monthly_Helmand, since=36, "Helmand", category="ACCESS")
...
quart_Be <- quarterly_AWARE_plots(pharmacy_Bentiu, monthly_Bentiu, since=36, "Bentiu")
